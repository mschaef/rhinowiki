...
<ul>
<li>An internal representation for compiled byte code functions.
<li>A way to interoperate with C code that expects binary data formats. 
    (<i>Like the Win32 API, for example.</i> )
<li>A way to represent binary data longer than a byte that's written to and
